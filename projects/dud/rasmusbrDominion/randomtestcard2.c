...
int failedEffects = 0, failedDiscards = 0, failedDraws = 0, failedHands = 0, failedActions = 0, failedCoins;

//function to check the smithyCard
void checkMinionCard(int players, struct gameState *endState){
        ...
        //call the card effect function with the minion card
        r = cardEffect(minion, 0, 0, 0, endState, 0, &bonus);
        //check if minion card failed
        if(r)
                failedEffects++;
...
int main (){
        ...
        for (n = 0; n < iterations; n++) {
                ...
                checkMinionCard(player,&randGame);
        }
